...
public class RubikCube implements Subject<RubikCube> {
    ...
    // had to alter this as well from be bound be ing 100 to 10 (would not run otherwise)
    // it is the amount of random motions it can undergo
    public void randomize(){
        Random random = new Random();
        int randomActionCount = random.nextInt(10);
    ...
    // had to add this to the main class to generate appropriate array for front and back tree
    // got help from Rebecca on this
    // she also helped us out with the main file in this folder to test the random actions and reverse actions
    public List<List<Integer>> generateArray(){
        List<List<Integer>> solution = new ArrayList<>();
        solution.add(this.main.getValueList());
        solution.add(this.right.getValueList());
        solution.add(this.left.getValueList());
        solution.add(this.back.getValueList());
        solution.add(this.top.getValueList());
        solution.add(this.bottom.getValueList());
        return solution;
    }
}
